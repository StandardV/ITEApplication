...
current_dir = os.path.dirname(os.path.abspath(__file__))
model_path = os.path.join(current_dir, "hyperlinkclassification_svm.pkl")
csv_path = os.path.join(current_dir,"DatasetCompany.csv")

# try:
data = pd.read_csv(csv_path)
data_subset = data[:1200]
print(data_subset.isnull().sum())
data_subset = data_subset.dropna()
...
# SVM classifier with a linear kernel (can experiment with other kernels)
...
training_set = [svm_classifier, tfidf_vectorizer]
with open(model_path, "wb") as f:
    joblib.dump(training_set, f)
